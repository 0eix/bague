...
Grid(n=2)
{
	...
	Diamant(n)
	{
		...
		for(let i=0;i<n;++i)
		{
			...
			let p = Vec3(0,Math.sin(alpha),Math.cos(alpha));
			cnorm.push(p);
			cpos.push(p.mult(0.2));
		}

		for(let j=0;j<n;++j)
		{
			let tr = rotate((360/n1)*j,Vec3(0,0,1)).mult(translate(0,0.1,0));
			let ntr = tr.inverse3transpose();
			const v = (1.0/n1)*j;
			for(let i=0;i<n;++i)
			{
	...
	Ring(n)
	{
		...
		for(let i=0;i<n;++i)
		{
			...
			let p = Vec3(0,Math.sin(alpha),Math.cos(alpha));
			cnorm.push(p);
			cpos.push(p.mult(0.02));
		}

		for(let j=0;j<n;++j)
		{
			let tr = rotate((360/n1)*j,Vec3(0,0,1)).mult(translate(0,0.3,0));
			let ntr = tr.inverse3transpose();
			const v = (1.0/n1)*j;
			for(let i=0;i<n;++i)
			{
